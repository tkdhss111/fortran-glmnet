...
module glmnet_wrapper
  ...
  ! Note: Using plain integer (not int32) to match -i8 flag (8-byte integers)
  type, public :: glmnet_result
    integer :: lmu = 0          ! number of lambda values
    integer :: jerr = 0         ! error flag
    integer :: nobs = 0         ! number of observations
    integer :: nvars = 0        ! number of predictors
    integer :: nlp = 0          ! number of passes over data
    integer, allocatable :: ia(:)      ! active variable indices
    integer, allocatable :: nin(:)     ! number of nonzero coefs per lambda
    ...
    integer :: lambda_min_idx = 0      ! index of min CV error
    integer :: lambda_1se_idx = 0      ! index of 1-SE rule
  contains
...
function glmnet_predict(fit, newx, s) result(yhat)
  ...
  integer, intent(in), optional :: s  ! which lambda (default: lambda_min or last)
...
subroutine find_optimal_lambdas(cvm, cvsd, idx_min, idx_1se)
  ...
  integer, intent(out) :: idx_min, idx_1se
  integer :: i, nlam
